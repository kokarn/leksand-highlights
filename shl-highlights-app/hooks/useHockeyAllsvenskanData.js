...
import {
    ...
    fetchHockeyAllsvenskanGameDetails,
    fetchHockeyAllsvenskanVideosForGame
} from '../api/shl';
 ...
 */
export function useHockeyAllsvenskanData(activeSport, selectedTeams, options = {}) {
    ...
    const loadGames = useCallback(async (silent = false) => {
        ...
        try {
            const data = await fetchHockeyAllsvenskanGames();
            setGames(data);
        } catch (e) {
            console.error("Failed to load HockeyAllsvenskan games", e);
        } finally {
    ...
    const loadStandings = useCallback(async (silent = false) => {
        ...
        try {
            const data = await fetchHockeyAllsvenskanStandings();
            setStandings(data);
        } catch (e) {
            console.error("Failed to load HockeyAllsvenskan standings", e);
        } finally {
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(() => {
            console.log('[HockeyAllsvenskan] Auto-refreshing live or starting-soon games...');
            loadGames(true);
    ...
    const handleGamePress = useCallback(async (game) => {
        ...
        const [details, vids] = await Promise.all([
            fetchHockeyAllsvenskanGameDetails(game.uuid),
            fetchHockeyAllsvenskanVideosForGame(game.uuid)
        ]);
    ...
    const refreshModalDetails = useCallback(async () => {
        ...
        try {
            const [details, vids] = await Promise.all([
                fetchHockeyAllsvenskanGameDetails(selectedGame.uuid),
                fetchHockeyAllsvenskanVideosForGame(selectedGame.uuid)
            ]);
            setGameDetails(details);
            const sortedVids = vids.sort((a, b) => {
                const aHigh = a.tags && a.tags.includes('custom.highlights');
            ...
            console.error('Failed to refresh HockeyAllsvenskan game details', error);
        } finally {
    ...
    useEffect(() => {
        ...
        const refreshDetails = async () => {
            console.log('[HockeyAllsvenskan] Auto-refreshing game details for live game...');
            const details = await fetchHockeyAllsvenskanGameDetails(selectedGame.uuid);
            setGameDetails(details);
        };
