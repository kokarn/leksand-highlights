...
import { fetchEuropaLeagueQualGames, fetchEuropaLeagueQualGameDetails } from '../api/shl';
...
/**
 * Hook for managing Europa League Qualifying data (schedule + match details only)
 ...
 */
export function useEuropaLeagueQualData(activeSport, selectedFootballTeams, options = {}) {
    ...
    const loadGames = useCallback(async (silent = false) => {
        ...
        try {
            const data = await fetchEuropaLeagueQualGames({ limit: 200 });
            setGames(Array.isArray(data) ? data : []);
        } catch (e) {
            console.error('Failed to load Europa League Qualifying games', e);
        } finally {
    ...
    const handleGamePress = useCallback(async (game) => {
        ...
        setGameDetails(null);
        try {
            const details = await fetchEuropaLeagueQualGameDetails(game.uuid);
            setGameDetails(details);
        } catch (error) {
            console.error('Failed to load Europa League Qualifying match details', error);
        } finally {
    ...
    const refreshModalDetails = useCallback(async () => {
        ...
        try {
            const details = await fetchEuropaLeagueQualGameDetails(selectedGame.uuid);
            setGameDetails(details);
        } catch (error) {
            console.error('Failed to refresh Europa League Qualifying match details', error);
        } finally {
