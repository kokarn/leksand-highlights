...
import { fetchConferenceLeagueQualGames, fetchConferenceLeagueQualGameDetails } from '../api/shl';
...
/**
 * Hook for managing Conference League Qualifying data (schedule + match details only)
 ...
 */
export function useConferenceLeagueQualData(activeSport, selectedFootballTeams, options = {}) {
    ...
    const loadGames = useCallback(async (silent = false) => {
        ...
        try {
            const data = await fetchConferenceLeagueQualGames({ limit: 200 });
            setGames(Array.isArray(data) ? data : []);
        } catch (e) {
            console.error('Failed to load Conference League Qualifying games', e);
        } finally {
    ...
    const handleGamePress = useCallback(async (game) => {
        ...
        setGameDetails(null);
        try {
            const details = await fetchConferenceLeagueQualGameDetails(game.uuid);
            setGameDetails(details);
        } catch (error) {
            console.error('Failed to load Conference League Qualifying match details', error);
        } finally {
    ...
    const refreshModalDetails = useCallback(async () => {
        ...
        try {
            const details = await fetchConferenceLeagueQualGameDetails(selectedGame.uuid);
            setGameDetails(details);
        } catch (error) {
            console.error('Failed to refresh Conference League Qualifying match details', error);
        } finally {
